...
void clear_variables();//good
void fileprocess ();//reading file good
void filewrite();//good
void bang_execute();
void environmment();//good
//void set_environment_variables();//omit
void change_directory();//good
void parent_directory();//good
...
char* skipcomma(char* str);//good
static int split(char *cmd_exec, int, int, int);//good
void with_pipe_execute();//good
static int command(int, int, int, char *cmd_exec);
void prompt();//good
void sigintHandler(int sig_num);//good
...
void fileprocess ()
{
  ...
  //S_IRUSR|S_IWUSR - read and write permissions for user
  int bytes_read=0, i=0, x=0, index=0;
  char buffer[1], temp_data[1000];
    do 
    {
        bytes_read = read (fd, buffer, sizeof (buffer));
		// read() function attempts to read n bytes from the file associated with fd, and places the characters read into a buffer
...
void filewrite()
{
  ...
  //no pointer to an array of char elements where the resulting C string is stored
  //no has number of lines
  strcpy(input_data, " ");
  ...
  if(input_buffer[i]=='!')
  ...
  else if(input_buffer[i]=='-')
    {
        ...
        while ((num_ch[n]=strtok(NULL,"-"))!=NULL)
              n++;
        num_ch[n]=NULL;
        num = atoi(num_ch[1]);//convert string to integer
...
void environmment()
{
  ...
  //searches for the environment string pointed to by its argument and returns the associated value to the string
...
void echo_calling(char *echo_val)
{
  ...
  memset(new_args, '\0', sizeof(new_args));//copies value 0 into new_args
  i=0; 
  while(str[1][i]!='\0')
    {
      if(str[1][i]=='"')
...
static int split(char *cmd_exec, int input, int first, int last)
{
    char *new_cmd_exec1;  
    new_cmd_exec1=strdup(cmd_exec);
   //it performs dynamic memory allocation. It returns a pointer to newly allocated memory into which it has copied the string.
...
//strtok breaks string str into a series of tokens using a delimiter
...
{   
    ...
    signal(SIGINT, sigintHandler);
    //SIGINT is an interrupt signal which the terminal sends to the foreground process when the user presses Ctrl+C
